...
def analyze_ecg_and_plot(file_path, window_size=10):
    ...
    try:
        ...
        try:
            ...
            Rwave_peaks_d_ecg, threshold = ekg.d_ecg_peaks(d_ECG, peaks_d_ecg, time, 0.3, 0.15)
            print(f"Versuch 2 erfolgreich: {len(Rwave_peaks_d_ecg)} R-Wave Peaks gefunden")
        except Exception as e2:
            print(f"Versuch 2 fehlgeschlagen: {e2}")
            try:
                # Dritter Versuch mit sehr niedrigen Schwellen
                Rwave_peaks_d_ecg, threshold = ekg.d_ecg_peaks(d_ECG, peaks_d_ecg, time, 0.2, 0.1)
                print(f"Versuch 3 erfolgreich: {len(Rwave_peaks_d_ecg)} R-Wave Peaks gefunden")
    ...
    print(f"Anzahl RR-Intervalle (nach Filterung): {len(RR_intervals_seconds)}")
    ...
    if len(windowed_hr) > 0:
        print(f"Zeit-Bereich: {windowed_times.min():.2f} - {windowed_times.max():.2f} ms")
        print(f"HR-Bereich: {windowed_hr.min():.2f} - {windowed_hr.max():.2f} bpm")
